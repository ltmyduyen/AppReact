...
/* ========= Random ổn định theo seed (seeded shuffle) ========= */
...
/** Shuffle có seed để ổn định theo chi nhánh + ngày */
...
async function fetchFoodsByIds(ids: string[]): Promise<Food[]> {
  ...
  for (const fid of ids) {
    ...
    const qy = query(collection(db, "foods"), where("id", "==", fid), limit(1));
...
const HomeScreen: React.FC = () => {
  ...
  /* Lấy món theo chi nhánh hiện tại: 
     ...
     - Thứ tự món được seeded-shuffle theo: branchId + YYYY-MM-DD (mỗi chi nhánh khác nhau, mỗi ngày đổi 1 lần)
  */
  useEffect(() => {
    ...
    (async () => {
      ...
      const seed = `${selectedBranch}-${todayKey()}`;

      const branchFoodsRef = collection(db, `branches/${selectedBranch}/branchFoods`);
      unsub = onSnapshot(branchFoodsRef, async (snapshot) => {
        try {
          ...
          let visibleFoods: Food[] = [];
          if (actives.length > 0) {
            const foodIds = actives.map((f: any) => String(f.foodId ?? f.id));
            visibleFoods = await fetchFoodsByIds(foodIds);
          } else {
            // Fallback: lấy toàn bộ foods
            const allFs = await getDocs(collection(db, "foods"));
            visibleFoods = allFs.docs.map(
              (docSnap) => ({ id: docSnap.id, ...(docSnap.data() as any) } as Food)
            );
          }

          // Random ổn định theo chi nhánh + ngày
          const shuffled = seededShuffle(visibleFoods, seed);

          setFoods(shuffled);

          // Áp dụng search nếu đang nhập
          if (searchQuery.trim()) {
